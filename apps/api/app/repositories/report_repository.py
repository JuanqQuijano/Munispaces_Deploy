...
class ReportRepository:
    ...
    def list_all(self, estado: ReportStatus | None = None) -> list[Report]:
    ...
    def stats_by_tipo(self) -> list[dict]:
        rows = self.db.execute(
            select(Report.tipo, func.count()).group_by(Report.tipo).order_by(func.count().desc())
        )
